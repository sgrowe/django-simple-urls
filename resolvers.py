...
def _validate_path_piece_name(name):
    if not name or not re.match(r'^[a-zA-Z_]+$', name):
        raise ImproperlyConfigured('Bad path piece name in simple_url pattern: {!r}.'
                                   ' Format should be either " piece_name " or '
                                   '" piece_name: type "'.format(name))
...
def _pattern_for_path_piece(path_piece_type):
    ...
    try:
        return path_type_patterns[path_piece_type]
    except KeyError:
        valid_types = ', '.join(repr(key) for key in path_type_patterns.keys())
